...
const handleAccordionClick = (index, accordion) => {
  ...
  return (
<div className="m-6 flex flex-row justify-center space-x-16" id="comparisonbox">
    ...
    <div>
      ...
      <div className="space-y-3">
        {[0, 1, 2, 3].map((index) => (
          <div
            key={index}
            className={`collapse collapse-plus ${index === selectedItem1 ? 'bg-accent' : 'bg-secondary'}  rounded-none`}
            onClick={() => {
    ...
    <div className='flex flex-row items-center py-3 space-x-8'>
      ...
      <div className="space-y-3">
        {[0, 1, 2, 3].map((index) => (
          <div
            key={index}
            className={`collapse collapse-plus ${index === selectedItem2 ? 'bg-accent' : 'bg-secondary'}  rounded-none`}
            onClick={() => {
              handleAccordionClick(index, 1);
              handleAccordionClick(index, 2);
            }}
